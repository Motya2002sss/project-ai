...
Press `Ctrl+C` once in that window to stop Expo, FastAPI, and the Quick Tunnel started by the launcher. PostgreSQL stays in Docker so the next launch is quick. Before the first combined launch, stop any old backend, tunnel, and Expo Terminal windows with `Ctrl+C`; the launcher refuses to start duplicate processes on ports `8000` or `8081`.
